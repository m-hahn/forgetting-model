...
for i in range(len(dundee)):
    ...
    print(Token)
    Itemno = (Itemno)
    if i == 0 or Itemno != dundee[i-1][header["Itemno"]]:
        print(calibrationSentences[-1:])
        calibrationSentences.append([])
        print(Itemno, dundee[i-1][header["Itemno"]])
...
#    print(plain_lm) 
    ...
    #          print(numerified.size())
              ...
              result, resultNumeric, fractions, thatProbs, amortizedPosterior = autoencoder.sampleReconstructions(numeric, numeric_noised, None, 2, numberOfBatches=numberOfSamples*2, fillInBefore=pointWhereToStart)
              # get THAT fractions
...
#              print(resultNumeric.size(), numeric_noised.size())
              ...
              batch_tokens = [([itos_total[resultNumeric_cpu[r,s]] if itos_total[resultNumeric_cpu[r,s]] != "OOV" else encoded_cropped[r] for r in range(pointWhereToStart+1, resultNumeric.size()[0])]) for s in range(resultNumeric.size()[1])]
              for h in range(len(batch_tokens)):
                 batch_tokens[h][0] = batch_tokens[h][0][:1].upper() + batch_tokens[h][0][1:]
                 assert batch_tokens[h][0][0] != " ", batch_tokens[h]
                 for i in range(1, len(batch_tokens[h])):
                     if batch_tokens[h][i-1] in [".", "?", "!"]:
                         batch_tokens[h][i] = batch_tokens[h][i][:1].upper() + batch_tokens[h][i][1:]
...
for epoch in range(1000):
   ...
   while updatesCount <= maxUpdates:
      ...
      if True:
       ...
       getTotalSentenceSurprisalsCalibration(SANITY="ZeroLoss")
       quit()
