...
tokenizer = AutoTokenizer.from_pretrained("transfo-xl-wt103", cache_dir="/u/scr/mhahn/cache/")


# add the EOS token as PAD token to avoid warnings
model = AutoModelWithLMHead.from_pretrained("transfo-xl-wt103", cache_dir="/u/scr/mhahn/cache/").cuda() # pad_token_id=tokenizer.eos_token_id, 
print("Finished loading TransformerXL")

#text = "Replace me by any text you'd like."
#encoded_input = tokenizer.encode(text, return_tensors='pt', add_space_before_punct_symbol=True)
#print(encoded_input)
...
#counter = 0
#with open("/jagupard27/scr0/mhahn/memory/char-lm-ud-stationary_12_SuperLong_WithAutoencoder_WithEx_Samples_Short_Combination_Subseq_VeryLong_WithSurp12_NormJudg_Short_CondTransformerXL.py_749792590_Model.txt", "r") as inFile:
#   next(inFile)
#   for line in inFile:
#     counter += 1
...
#       print(tokenizer.bos_token_id, tokenizer.eos_token_id) they all evaoluate to 50256, the ID of <|endoftext|>
  #     quit()
       ...
       for i in range(len(tensors)):
          tensors[i] = torch.cat([torch.LongTensor([tokenizer.eos_token_id]).view(1,1), tensors[i], torch.LongTensor([tokenizer.eos_token_id for _ in range(maxLength - tensors[i].size()[1])]).view(1, -1)], dim=1)
...
#         print("\t".join([batch[batchElem], str( sum([sum(x[1] for x in y) for y in words[:-1]])), str(sum(x[1] for x in words[-1]))]))
 #      quit()  
#         TransformerXLsurprisals[batch[batchElem]] = ( sum([sum(x[1] for x in y) for y in words[:-1]]), sum(x[1] for x in words[-1]))
#       quit()
