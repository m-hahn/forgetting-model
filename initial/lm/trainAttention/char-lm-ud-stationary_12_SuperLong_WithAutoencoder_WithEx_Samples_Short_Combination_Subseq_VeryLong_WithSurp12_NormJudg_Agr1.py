...
def getTotalSentenceSurprisalsCalibration(SANITY="Sanity", VERBS=2): # Surprisal for EOS after 2 or 3 verbs
    ...
    numberOfSamples = 12
    with torch.no_grad():
     with open("/u/scr/mhahn/reinforce-logs-both-agr1/calibration-full-logs-tsv/"+__file__+"_"+str(args.myID)+"_"+SANITY, "w") as outFile:
...
def getTotalSentenceSurprisals(SANITY="Model", VERBS=2): # Surprisal for EOS after 2 or 3 verbs
    ...
    with torch.no_grad():
      for NOUN in topNouns:
         NOUNPlural = NOUN + "s"
         if NOUNPlural.endswith("ys"):
           NOUNPlural = NOUNPlural[:-2]+"ies"

         print(NOUN, NOUNPlural, "Time:", time.time() - startTimePredictions, file=sys.stderr)
         if NOUNPlural not in stoi:
            continue
...
#              print(totalSurprisal.size())
    ...
    with open("/u/scr/mhahn/reinforce-logs-both-agr1/full-logs-tsv/"+__file__+"_"+str(args.myID)+"_"+SANITY, "w") as outFile:
      print("Noun", "Region", "Condition", "Surprisal", "ThatFraction", file=outFile)
      for noun in topNouns:
...
for epoch in range(1000):
   ...
   while updatesCount <= maxUpdates:
      ...
      if updatesCount == maxUpdates:
       ...
       with open("/u/scr/mhahn/reinforce-logs-both-agr1/full-logs/"+__file__+"_"+str(args.myID), "w") as outFile:
         startTimePredictions = time.time()
...
with open("/u/scr/mhahn/reinforce-logs-both-agr1/results/"+__file__+"_"+str(args.myID), "w") as outFile:
