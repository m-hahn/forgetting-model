...
ggsave(plot, file="staub_2016_vanillaLSTM_smoothed.pdf", height=10, width=10)


# matrix verb
plot = ggplot(data=surprisalSmoothed %>% filter(Region == "V1"), aes(x=Group, y=SurprisalReweighted, group=Length, fill=Length, color=Length)) + geom_bar(stat="identity", width=.5, position = "dodge") + facet_grid(predictability_weight ~ deletion_rate) + theme_bw()
...
plot = ggplot(data=surprisalSmoothed %>% filter(deletion_rate == 0.1 | deletion_rate == 0.55) %>% group_by(deletion_rate, Length, HasParticle, Region, RCType, Condition, HasPP, Group) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted)) %>% filter(Region == "V1"), aes(x=Group, y=SurprisalReweighted, group=Length, fill=Length, color=Length)) + geom_bar(stat="identity", width=.5, position = "dodge") + facet_grid( ~ deletion_rate) + theme_bw()
ggsave(plot, file="figures/staub_2016_vanillaLSTM_smoothed_MatrixVerb_selected.pdf", height=5, width=10)
...
plot = ggplot(data=surprisalSmoothed %>% filter(deletion_rate == 0.1 | deletion_rate == 0.55) %>% group_by(deletion_rate, HasParticle, Region, RCType, Condition, HasPP) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted)) %>% filter(Region != "V1", !HasParticle), aes(x=Region, y=SurprisalReweighted, group=Condition, color=RCType, linetype=HasPP)) + geom_line() + facet_grid( ~ deletion_rate) + theme_bw()
ggsave(plot, file="figures/staub_2016_vanillaLSTM_smoothed_Inside_selected.pdf", height=5, width=10)
