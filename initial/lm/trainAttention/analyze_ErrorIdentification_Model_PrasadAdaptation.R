...
data = read.csv("../../../../..//reinforce-logs-both-short/stimuli-full-logs-tsv-EIS/collectResults_Stims_ErrorIdentification.py_GrodnerEtAl2003_Model.tsv", sep="\t")
...
#ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_ToyGrodnerEtAl2003.pdf", height=15, width=40)
...
#ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Toy_SurprisalGrodnerEtAl2003.pdf", height=15, width=40)
#
...
#data$Ambiguous = grepl("ambig" , data$Condition) - 0.5
...
#
#sink("output/analyze_ErrorIdentification_Model.RGrodnerEtAl2003.tsv")
...
data2 = data%>%filter(Script == "errorIdentification_Erasure4.py", Region %in% c("verb2_0")) %>% group_by(deletion_rate, predictability_weight, Item, Condition) %>% summarise(EISReweighted=mean(EISReweighted), SurprisalReweighted=mean(SurprisalReweighted))
...
ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Tabor_EISGrodnerEtAl2003.pdf", height=10, width=15)
...
ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Tabor_SurprisalGrodnerEtAl2003.pdf", height=10, width=15)

data2 = data%>%filter(Script == "errorIdentification_Erasure4.py", Region %in% c("post1_0")) %>% group_by(deletion_rate, predictability_weight, Item, Condition) %>% summarise(EISReweighted=mean(EISReweighted), SurprisalReweighted=mean(SurprisalReweighted))


plot = ggplot(data2 %>% group_by(deletion_rate, predictability_weight, Condition) %>% summarise(EISReweighted=mean(EISReweighted)), aes(x=Condition, y=EISReweighted, color=Condition)) + geom_point() + facet_grid(predictability_weight~deletion_rate)
ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Tabor_EISGrodnerEtAl2003_Spillover.pdf", height=10, width=15)
plot = ggplot(data2 %>% group_by(deletion_rate, predictability_weight, Condition) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted)), aes(x=Condition, y=SurprisalReweighted, color=Condition)) + geom_point() + facet_grid(predictability_weight~deletion_rate)
ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Tabor_SurprisalGrodnerEtAl2003_Spillover.pdf", height=10, width=15)
