assert False, "have to sanity-check this, in particular regarding cutting off the final word. the model is trained without any curring done (retentioin probability on the last word is not zero)"
#~/python-py37-mhahn errorIdentification_DeletionLM3_NoSanity.py --load-from-joint=98331529 --stimulus_file=tabor_2004_expt1_3_tokenized --criticalRegion=participle_0
...
if args.load_from_joint is not None:
 # try:
  ...
  checkpoint = torch.load("/u/scr/mhahn/CODEBOOKS_memoryPolicy_both/"+args.language+"_"+"autoencoder2_mlp_bidir_Deletion_Reinforce2_Tuning_Long_Both_Saving_Lagrange_BoundarySymbol_NoPunct_WithLM.py"+"_code_"+str(args.load_from_joint)+".txt")
