...
def getTotalSentenceSurprisalsCalibration(SANITY="Sanity", VERBS=2): # Surprisal for EOS after 2 or 3 verbs
    ...
    numberOfSamples = 12
    with torch.no_grad():
     with open("/u/scr/mhahn/reinforce-logs-both/calibration-full-logs-tsv/"+__file__+"_"+str(args.myID)+"_"+SANITY, "w") as outFile:
...
def getTotalSentenceSurprisals(SANITY="Sanity", VERBS=2): # Surprisal for EOS after 2 or 3 verbs
    assert SANITY in ["Sanity", "Model"]
    assert VERBS in [1,2]
    print(plain_lm) 
    surprisalsPerNoun = {}
...
#    topNouns = ["fact", "report"]
    with torch.no_grad():
      for NOUN in topNouns:
         ...
         thatFractionsCount = {x : defaultdict(float) for x in ["g", "u"]}
         surprisalCountByRegions = {x : defaultdict(float) for x in ["g", "u"]}
         ...
         for sentenceList in nounsAndVerbs:
           ...
           for condition in ["g", "u"]: # ,"u" removing u for now to speed things up
            if condition == "g":
               remainingInput = f"{sentenceList[3]} {sentenceList[4]} .".split(" ")
               regions = flatten([[f"{region}_{c}" for c, _ in enumerate(words.split(" "))] for region, words in [("V2", sentenceList[3]), ("V1", sentenceList[4]), ("EOS", ".")]])
               assert len(remainingInput) == len(regions)
            else:
               remainingInput = f"{sentenceList[4]} .".split(" ")
               regions = flatten([[f"{region}_{c}" for c, _ in enumerate(words.split(" "))] for region, words in [("V1", sentenceList[4]), ("EOS", ".")]])
               assert len(remainingInput) == len(regions)
            for i in range(len(remainingInput)):
              numerified = encodeContextCrop(" ".join(remainingInput[:i+1]), context)
              assert numerified.size()[0] == args.sequence_length+1, (numerified.size())
...
#              print(totalSurprisal.size())
    ...
    for region in ["V2_0", "V1_0", "EOS_0"]:
       print(SANITY, "CORR", region, correlation(torch.FloatTensor([thatBias(x) for x in topNouns]), torch.FloatTensor([surprisalsPerNoun[x]["g"][region] for x in topNouns])), correlation(torch.FloatTensor([thatBias(x) for x in topNouns]), torch.FloatTensor([thatFractionsPerNoun[x]["g"][region] for x in topNouns])))
    overallSurprisalForCompletion = torch.FloatTensor([sum([surprisalsPerNoun[noun]["g"][region] for region in surprisalsPerNoun[noun]["g"]]) for noun in topNouns])
  ...
  #            print("FED INTO LM", " ".join([itos_total[int(x)] for x in resultNumeric[:,2*sampleDim].cpu().numpy()]))
   #           print("FED INTO LM", " ".join())
              ...
              for prompt in text:
                 ...
                 print(SANITY, NOUN,"\t", (sentenceList[0]), "\t", prompt)
              #quit()
