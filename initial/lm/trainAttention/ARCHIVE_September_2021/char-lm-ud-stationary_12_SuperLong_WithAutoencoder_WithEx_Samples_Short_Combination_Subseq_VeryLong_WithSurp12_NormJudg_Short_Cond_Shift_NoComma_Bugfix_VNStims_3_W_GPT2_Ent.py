...
## Learning Rates
parser.add_argument("--learning_rate_memory", type = float, default= random.choice([0.000002, 0.00001, 0.00002, 0.00005]))  # Can also use 0.0001, which leads to total convergence to deterministic solution withtin maximum iterations (March 25, 2021). But it's not clear this is actually good for the variational bound -> check!   #, 0.0001, 0.0002 # 1e-7, 0.000001, 0.000002, 0.000005, 0.000007, 
...
def get_entropy(counts):
  ...
  return -sum([(x/sum_counts)*math.log(x/sum_counts+1e-50) for x in counts])
...
#    topNouns = ["fact", "report"]
    entropiesPerPosition = [[0.0, 0] for _ in range(args.sequence_length+1)]
    with torch.no_grad():
      TRIALS_COUNT = 0
...
#              print(log_importance_weights_maxima)
              print(resultNumeric_cpu.size())
              
              resultNumeric_cpu_3d = resultNumeric_cpu.view(args.sequence_length+1, numberOfSamples, 24)
              log_importance_weights_cpu = log_importance_weights.detach().cpu()
              log_importance_weights_maxima_cpu = log_importance_weights_maxima.detach().cpu().view(numberOfSamples)
              for j in range(pointWhereToStart, args.sequence_length):
                  wordsHere = resultNumeric_cpu_3d[j]
 #                 print(wordsHere.size(), log_importance_weights_cpu.size())
                  entropies = [None for _ in range(numberOfSamples)]
                  for w in range(numberOfSamples):
                     counts = []
                     stoi_counts = {}
                     for q in range(24):
                         wordHere = int(resultNumeric_cpu_3d[j,w,q])
#                         print(log_importance_weights_maxima.size())
                         if wordHere not in stoi_counts:
                            stoi_counts[wordHere] = len(counts)
                            counts.append(0)
                         wordHere_i = stoi_counts[wordHere]
                         counts[wordHere_i] += math.exp(log_importance_weights_cpu[w,q] - float(log_importance_weights_maxima_cpu[w]))
                     entropies[w] = get_entropy(counts)
                  average_entropy = sum(entropies)/len(entropies)
                  entropiesPerPosition[j][0] += average_entropy
                  entropiesPerPosition[j][1] += 1
#              quit()
...
#              if i == 0 or regions[i] != regions[i-1]:
    ...
    with open("/u/scr/mhahn/reinforce-logs-both-short/full-logs-ent/"+__file__+"_"+str(args.myID)+"_"+SANITY, "w") as outFile:
         print(entropiesPerPosition)
         for x, z in enumerate(entropiesPerPosition):
            print("\t".join([str(q) for q in [x, z[0], z[1]]]), file=outFile)
