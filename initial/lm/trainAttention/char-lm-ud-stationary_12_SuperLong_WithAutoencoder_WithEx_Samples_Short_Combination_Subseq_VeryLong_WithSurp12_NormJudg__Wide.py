...
def prepareDatasetChunks(data, train=True, batchSize=args.batchSize):
      numeric = [0]
      count = 0
...
#         numerified_chars.append([0] + [stoi_chars[x]+3 if x in stoi_chars else 2 for x in char])
       ...
       if len(numerified) > (batchSize*(args.sequence_length+1)):
         sequenceLengthHere = args.sequence_length+1

         cutoff = int(len(numerified)/(batchSize*sequenceLengthHere)) * (batchSize*sequenceLengthHere)
...
#         numerified_chars = numerified_chars[cutoff:]
         ...
         numerifiedCurrent = torch.LongTensor(numerifiedCurrent).view(batchSize, -1, sequenceLengthHere).transpose(0,1).transpose(1,2).cuda()
...
nounsAndVerbs = []
nounsAndVerbs.append(["the principal",       "the teacher",        "kissed",      "appeared on tv",                     "was quoted in the newspaper", "Was the XXXX quoted in the newspaper?", "Y"])
nounsAndVerbs.append(["the sculptor",        "the painter",        "admired",    "surprised the doctor",   "was completely untrue", "Was the XXXX untrue?", "Y"])
nounsAndVerbs.append(["the consultant",      "the artist",         "hired",      "was confirmed",       "shocked everyone", "Did the XXXX shock everyone?", "Y"])
nounsAndVerbs.append(["the runner",          "the psychiatrist",   "treated",    "was credible",        "was ridiculous", "Was the XXXX ridiculous?", "Y"])
nounsAndVerbs.append(["the child",           "the medic",          "rescued",    "made people happy",      "relieved everyone", "Did the XXXX relieve everyone?", "Y"])
nounsAndVerbs.append(["the criminal",        "the officer",        "arrested",   "was refuted",        "was entirely bogus", "Was the XXXX bogus?", "Y"])
nounsAndVerbs.append(["the student",         "the professor",      "hated",      "shocked his colleagues",       "made the professor happy", "Did the XXXX make the professor happy?", "Y"])
nounsAndVerbs.append(["the mobster",         "the media",          "portrayed",  "calmed everyone down",    "turned out to be true", "Did the XXXX turn out to be true?", "Y"])
nounsAndVerbs.append(["the actor",           "the starlet",        "loved",      "was quoted in newspapers",       "made her cry", "Did the XXXX almost make her cry?", "Y"])
nounsAndVerbs.append(["the preacher",        "the parishioners",   "fired",      "was foolish",        "proved to be true", "Did the XXXX prove to be true?", "Y"])
nounsAndVerbs.append(["the violinist",       "the sponsors",       "backed",     "made her cry",                       "is likely true", "Was the XXXX likely true?", "Y"])
nounsAndVerbs.append(["the senator",         "the diplomat",       "opposed",    "annoyed him",                   "really made him angry", "Did the XXXX make him angry?", "Y"])
nounsAndVerbs.append(["the commander",       "the president",      "appointed",  "was dangerous",         "troubled people", "Did the XXXX trouble people?", "Y"])
nounsAndVerbs.append(["the victim",         "the criminal",       "assaulted",  "remained hidden",         "calmed everyone down", "Did the XXXX calm everyone down?", "Y"])
nounsAndVerbs.append(["the politician",      "the banker",         "bribed",     "was popular",         "came as a shock to his supporters", "Did the XXXX come as a shock?", "Y"])
nounsAndVerbs.append(["the surgeon",         "the patient",        "thanked",    "was widely known",         "was not a surprise", "Was the XXXX unsurprising?", "Y"])
nounsAndVerbs.append(["the extremist",       "the agent",          "caught",     "stunned everyone",         "was disconcerting", "Was the XXXX disconcerting?", "Y"])
nounsAndVerbs.append(["the clerk",           "the customer",       "called",     "was idiotic",         "seemed absurd", "Did the XXXX seem absurd?", "Y"])
nounsAndVerbs.append(["the trader",          "the businessman",    "consulted",  "sounded hopeful",         "was confirmed", "Was the XXXX confirmed?", "Y"])
nounsAndVerbs.append(["the CEO",             "the employee",       "impressed",  "hurt him",         "was entirely correct", "Was the XXXX correct?", "Y"])
...
def getFrequencyRanked(x):
   counts = defaultdict(int)
   for y in x:
     counts[itos_total[int(y)]] += 1
   counts = sorted(list(counts.items()), key=lambda x:x[1], reverse=True)
   return "  ".join([q[0]+"#"+str(q[1]) for q in counts[:5]])
...
def getTotalSentenceSurprisals(SANITY="Model", VERBS=2): # Surprisal for EOS after 2 or 3 verbs
    ...
    print("Got data")
    dev_chars = prepareDatasetChunks(dev_data, train=False, batchSize=1)
    with open("/u/scr/mhahn/reinforce-logs-both-wide/full-logs-tsv/"+__file__+"_"+str(args.myID), "w") as outFile:
     print(args, file=outFile)
     count = 0
     for numerified, _ in dev_chars:
       count += 1
       if count % 100 == 0:
         print(count)
       numeric, numeric_noised = forward(numerified, train=False, printHere=False, provideAttention=False, onlyProvideMemoryResult=True, NUMBER_OF_REPLICATES=numberOfSamples)
       ...
       result, resultNumeric, fractions, thatProbs = autoencoder.sampleReconstructions(numeric, numeric_noised, "fact", 2, numberOfBatches=numberOfSamples*24)
       numerified_ = numerified.detach().cpu()
       resultNumeric_ = resultNumeric.detach().cpu()
       for j in range(1, args.sequence_length):
         target = int(numerified_[j,0])
        
         if target < 100 and target != stoi_total["OOV"] and len(itos_total[target]) > 1:
            reconstructedFraction = float((resultNumeric_[:,j] == target).float().mean())
            if reconstructedFraction<0.2:
               print("\t".join([str(reconstructedFraction), itos_total[target], " ".join([itos_total[int(numerified_[i,0])] for i in range(args.sequence_length+1)]), str(j), getFrequencyRanked(resultNumeric_[:,j])]), file=outFile)
...
for epoch in range(1000):
   ...
   while updatesCount <= maxUpdates:
      ...
      if updatesCount == maxUpdates:
       ...
       if True: #with open("/u/scr/mhahn/reinforce-logs-both-short/full-logs/"+__file__+"_"+str(args.myID), "w") as outFile:
         startTimePredictions = time.time()
