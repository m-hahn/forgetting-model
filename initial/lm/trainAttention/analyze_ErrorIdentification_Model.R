...
ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Toy.pdf", height=15, width=40)
...
ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Toy_Surprisal.pdf", height=15, width=40)
...
#model = (lmer(EISReweighted ~ Ambiguous * Short + (1|ID) + (1|Item), data=data%>%filter(Script == "errorIdentification_Erasure4.py", Region %in% c("participle_0"))))
#print(summary(model))
...
data2 = data%>%filter(Script == "errorIdentification_Erasure4.py", Region %in% c("participle_0")) %>% group_by(deletion_rate, predictability_weight, Item, Condition, Ambiguous, Short) %>% summarise(EISReweighted=mean(EISReweighted), SurprisalReweighted=mean(SurprisalReweighted))

sink("output/analyze_ErrorIdentification_Model.R.tsv")
...
ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Tabor_EIS.pdf", height=10, width=15)
...
ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Tabor_Surprisal.pdf", height=10, width=15)



data2 = data%>%filter(Script == "errorIdentification_Erasure4.py", Region %in% c("participle_0")) %>% group_by(deletion_rate, predictability_weight, Item, Ambiguous, Short) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted))
for(i in 1:nrow(config)) {
  delta = config$deletion_rate[[i]]
  lambda = config$predictability_weight[[i]]
  u = data2 %>% filter(deletion_rate==delta, predictability_weight==lambda)
