...
verbs = ["mentioned", "described", "ignored", "discovered"]
participles = ["brought", "painted", "sent", "allowed", "told", "offered", "taught", "served", "dyed", "cut", "knitted", "rented", "nabbed", "recited", "brought", "tossed", "knitted", "mailed", "saved", "planted"]
...
def getTotalSentenceSurprisalsCalibration(SANITY="Sanity", VERBS=2): # Surprisal for EOS after 2 or 3 verbs
    ...
    numberOfSamples = 12
    with torch.no_grad():
     with open("/u/scr/mhahn/reinforce-logs-both-coh/calibration-full-logs-tsv/"+__file__+"_"+str(args.myID)+"_"+SANITY, "w") as outFile:
...
#    for region in ["V3", "V2", "V1", "EOS"]:
#       print(SANITY, "CORR", region, correlation(torch.FloatTensor([(float(counts[x][header["True_False"]])-float(counts[x][header["False_False"]])) for x in topNouns]), torch.FloatTensor([surprisalsPerNoun[x]["sing"][region]-surprisalsPerNoun[x]["plur"][region] for x in topNouns])), correlation(torch.FloatTensor([(float(counts[x][header["True_False"]])-float(counts[x][header["False_False"]])) for x in topNouns]), torch.FloatTensor([thatFractionsPerNoun[x]["sing"][region]-thatFractionsPerNoun[x]["plur"][region] for x in topNouns])))
#    overallSurprisalForCompletion = torch.FloatTensor([sum([surprisalsPerNoun[noun]["plur"][region] - surprisalsPerNoun[noun]["sing"][region] for region in ["V2", "V1", "EOS"]]) for noun in topNouns])
...
def getTotalSentenceSurprisals(SANITY="Model", VERBS=2): # Surprisal for EOS after 2 or 3 verbs
    ...
    numberOfSamples = 12
    from collections import defaultdict
    forOfPerNoun = defaultdict(list)
    condition = "of"
    with torch.no_grad():
      for NOUN in topNouns:
         print(NOUN, "Time:", time.time() - startTimePredictions, file=sys.stderr)
         for participle in participles:           
          if participle not in stoi:
              continue
          for sentenceList in nounsAndVerbs:
           verb = "_"
           while verb not in stoi:
              verb = random.choice(verbs)
           stimulus = [sentenceList[0], verb, "the", NOUN, "of", sentenceList[1], participle, "a"]
           #print(sentenceList)
           thatFractions = {x : defaultdict(float) for x in ["of"]}
           surprisalByRegions = {x : defaultdict(float) for x in ["of"]}

           if True:
               
            context = "later , the nurse suggested they treat the patient with an antibiotic, but in the end , this did not happen . " + " ".join(stimulus[:-2])

            remainingInput = [stimulus[-2]]
            regions = ["V2_0"]
    ...
    #          print(numerified.size())
              ...
              forOfPerNoun[NOUN] += [itos_total[int(x)] for x in resultNumeric[:,-4].cpu()]
#              print([itos_total[int(x)] for x in resultNumeric[:,-4].cpu()])
              resultNumeric = resultNumeric.transpose(0,1).contiguous()
              nextWord = torch.LongTensor([stoi_total.get(remainingInput[i], stoi_total["OOV"]) for _ in range(numberOfSamples*24)]).unsqueeze(0).cuda()
              resultNumeric = torch.cat([resultNumeric[:-1], nextWord], dim=0).contiguous()
...
#              print(totalSurprisal.size())
    ...
    with open("/u/scr/mhahn/reinforce-logs-both-coh/forOfPerNoun_"+__file__+"_"+str(args.myID), "w") as outFile:
       for NOUN in forOfPerNoun:
          counts = defaultdict(int)
          for x in forOfPerNoun[NOUN]:
              counts[x] += 1
          for x in counts:
             print("\t".join([NOUN, x, str(counts[x])]), file=outFile)
    with open("/u/scr/mhahn/reinforce-logs-both-coh/full-logs-tsv/"+__file__+"_"+str(args.myID)+"_"+SANITY, "w") as outFile:
      print("Noun", "Region", "Condition", "Surprisal", "ThatFraction", file=outFile)
      for noun in topNouns:
       if noun not in surprisalsPerNoun:
           continue
       for condition in ["of"]:
 ...
 #   for region in ["V2_0"]:
#       print(SANITY, "CORR", region, correlation(torch.FloatTensor([thatBias(x) for x in topNouns]), torch.FloatTensor([surprisalsPerNoun[x]["of"][region] for x in topNouns])), correlation(torch.FloatTensor([thatBias(x) for x in topNouns]), torch.FloatTensor([thatFractionsPerNoun[x]["of"][region] for x in topNouns])))
    overallSurprisalForCompletion = torch.FloatTensor([sum([surprisalsPerNoun[noun]["of"][region]  for region in surprisalsPerNoun[noun]["of"]]) for noun in topNouns])
...
for epoch in range(1000):
   ...
   while updatesCount <= maxUpdates:
      ...
      if updatesCount == maxUpdates:
       ...
       with open("/u/scr/mhahn/reinforce-logs-both-coh/full-logs/"+__file__+"_"+str(args.myID), "w") as outFile:
         startTimePredictions = time.time()
...
with open("/u/scr/mhahn/reinforce-logs-both-coh/results/"+__file__+"_"+str(args.myID), "w") as outFile:
