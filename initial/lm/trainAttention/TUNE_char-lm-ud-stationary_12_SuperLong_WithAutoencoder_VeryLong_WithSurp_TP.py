...
#   predictability_weight=str(random.choice([0.5, 1.0]))
   command = ["/u/nlp/anaconda/main/anaconda3/envs/py37-mhahn/bin/python", random.choice(scripts), "--tuning=1", "--deletion_rate="+deletion_rate, "--predictability_weight="+predictability_weight, "--learning_rate_memory="+str(random.choice([0.00002, 0.00005, 0.0001])), "--learning_rate_autoencoder="+str(random.choice([0.001, 0.01, 0.1, 0.1, 0.1, 0.1, 0.2])), "--momentum="+str(random.choice([0.5, 0.7, 0.7, 0.7, 0.7, 0.9]))] #, "--predictability_weight=0.0"]
   print(command)
