...
#           self.beginning = torch.where(forRestart.unsqueeze(0) == 1, zeroBeginning, self.beginning)
       ...
       #assert numeric.size()[0] == args.sequence_length+1 # I've removed this assumption to get flexible-length posterior samples cutting off the prefix
...
class Autoencoder:
  ...
  def sampleReconstructions(self, numeric, numeric_noised, NOUN, offset, numberOfBatches=args.batchSize*args.NUMBER_OF_REPLICATES, computeProbabilityStartingFrom=0):
...
#          print(i, probs[0,:, stoi["that"]+3].mean())
 #         quit()
          ...
          if computeProbabilityStartingFrom <= i:
...
#            print(i, [itos_total[int(x)] for x in numeric_noised[i].view(-1)])
  #          quit()
#          nextWord = (dist.sample())
...
def forward(numeric, train=True, printHere=False, provideAttention=False, onlyProvideMemoryResult=False, NUMBER_OF_REPLICATES=args.NUMBER_OF_REPLICATES, expandReplicates=True, computeProbabilityStartingFrom=0):
...
#      print([itos_total[int(x)] for x in numeric[computeProbabilityStartingFrom]])
      if onlyProvideMemoryResult:
        return numeric, numeric_noised, bernoulli_logprob[computeProbabilityStartingFrom:].sum(dim=0)
...
nounsAndVerbsCompatible.append(["the cousin", "the brother", "described", "troubled the uncle", "startled the mother."])
nounsAndVerbsIncompatible.append(["the cousin", "the brother", "described", "killed the uncle", "startled the mother."])
     ...
     with open("/u/scr/mhahn/reinforce-logs-both-short/calibration-full-logs-tsv/"+__file__+"_"+str(args.myID)+"_"+SANITY, "w") as outFile:
      ...
      for sentence in calibrationSentences:
            ...
            for i in range(len(remainingInput)):
              ...
              # Run the noise model
              numeric, numeric_noised, _ = forward(numerified, train=False, printHere=False, provideAttention=False, onlyProvideMemoryResult=True, NUMBER_OF_REPLICATES=numberOfSamples)
...
def getTotalSentenceSurprisals(SANITY="Model", VERBS=2): # Surprisal for EOS after 2 or 3 verbs
     ...
     with torch.no_grad():
      for NOUN in topNouns:
        ...
        for compatible in ["compatible", "incompatible"]:
         for sentenceIndex, sentenceList in enumerate({"compatible" : nounsAndVerbsCompatible, "incompatible" : nounsAndVerbsIncompatible}[compatible]):
           print(sentenceList)
    ...
    #          print(numerified.size())
              ...
              if SANITY == "Sanity":
                 ...
                 assert SANITY == "Model"
                 numeric, numeric_noised, likelihoods = forward(numerified, train=False, printHere=False, provideAttention=False, onlyProvideMemoryResult=True, NUMBER_OF_REPLICATES=numberOfSamples, computeProbabilityStartingFrom=pointWhereToStart+1)
                 numeric_noised = torch.where(numeric == stoi["."]+3, numeric, numeric_noised)
              ...
              result, resultNumeric, fractions, thatProbs, amortizedPosterior = autoencoder.sampleReconstructions(numeric, numeric_noised, NOUN, 2, numberOfBatches=numberOfSamples*24, computeProbabilityStartingFrom=pointWhereToStart)
              for q in range(numberOfSamples):
                for r in range(24):
                   # TODO for reducing variance, it mighr be useful to restrict the likelihood and amortized posterior to the relevant words
#                   print(pointWhereToStart, args.sequence_length, len(context.split(" ")) , i , 1)
