...
import sampleNextWordFromRoBERTa
#quit()
...
#      print(memory.relu(memory.memory_mlp_inner_bilinear(embedded_everything.detach())).transpose(1,2).size())
      ...
      if args.predictability_weight > 0:
       ...
       # Prediction Loss 
       lm_lossTensor = lm.print_loss(lm_log_probs.view(-1, len(itos)+3), target_tensor_full[-1].view(-1)).view(-1, NUMBER_OF_REPLICATES) # , args.batchSize is 1
      ##########################################
      ##########################################
...
def sampleReconstructions(numeric, numeric_noised, NOUN, offset, numberOfBatches=args.batchSize*args.NUMBER_OF_REPLICATES):
      if True:
          beginning = zeroBeginning
 ...
 #             print("FED INTO LM", " ".join([itos_total[int(x)] for x in resultNumeric[:,1*sampleDim].cpu().numpy()]))
  #            print("FED INTO LM", " ".join([itos_total[int(x)] for x in resultNumeric[:,2*sampleDim].cpu().numpy()]))
   #           print("FED INTO LM", " ".join())
              text = [" ".join([itos_total[int(x)] for x in resultNumeric[:-1,i].cpu().numpy()]) for i in range(numberOfSamples*sampleDim)]
              finished = []
              for i in range(20):
                 if len(text) == 0:
                   break
                 print(i, text[0])
                 nextWords = sampleNextWordFromRoBERTa.sampleNextWord(text, contractSampleDimension = None)
                 for j in range(len(text)):
                     text[j] += nextWords[j]
                     if nextWords[j] in [".", "?", "!", "</s>"]:
                        finished.append(text[j])
                 text = [text[q] for q in range(len(text)) if nextWords[q] not in [".", "?", "!", "</s>"]]
              finished += text
              for j in range(len(finished)):
                    print("GENERATED", NOUN, j, SANITY, finished[j])

#              samplesFromLM = [x.strip() for x in nextWords]
#              print(nextWords)
#
#              print(numerified.size(), len(samplesFromLM))
#              numerified = torch.cat([numerified[1:-1], torch.LongTensor([stoi_total.get(x.lower(), stoi_total["OOV"]) for x in samplesFromLM]).unsqueeze(0).cuda()], dim=0)
