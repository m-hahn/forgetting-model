...
def getTotalSentenceSurprisalsCalibration(SANITY="Sanity", VERBS=2): # Surprisal for EOS after 2 or 3 verbs
    ...
    numberOfSamples = 12
    with torch.no_grad():
     with open("/u/scr/mhahn/reinforce-logs-both-agr2/calibration-full-logs-tsv/"+__file__+"_"+str(args.myID)+"_"+SANITY, "w") as outFile:
    ...
    with torch.no_grad():
      for NOUN in topNouns:
         ...
         for sentenceList in nounsAndVerbs:
           embeddedNoun = sentenceList[0]
           embeddedNounPlural = embeddedNoun+"s"
           print(sentenceList)
           if embeddedNounPlural.split(" ")[-1] not in stoi:
               continue
           thatFractions = {x : defaultdict(float) for x in ["sing", "plur"]}
           surprisalByRegions = {x : defaultdict(float) for x in ["sing", "plur"]}

           for condition in ["sing","plur"]:
            if condition == "sing":
               embeddedNounPerNumber = embeddedNoun
            else:
               embeddedNounPerNumber = embeddedNounPlural
               
            context = "later , the nurse suggested they treat the patient with an antibiotic, but in the end , this did not happen . " + f"the {NOUN} of {embeddedNounPerNumber}"
...
#              print(totalSurprisal.size())
    ...
    with open("/u/scr/mhahn/reinforce-logs-both-agr2/full-logs-tsv/"+__file__+"_"+str(args.myID)+"_"+SANITY, "w") as outFile:
      print("Noun", "Region", "Condition", "Surprisal", "ThatFraction", file=outFile)
      for noun in topNouns:
...
for epoch in range(1000):
   ...
   while updatesCount <= maxUpdates:
      ...
      if updatesCount == maxUpdates:
       ...
       with open("/u/scr/mhahn/reinforce-logs-both-agr2/full-logs/"+__file__+"_"+str(args.myID), "w") as outFile:
         startTimePredictions = time.time()
...
with open("/u/scr/mhahn/reinforce-logs-both-agr2/results/"+__file__+"_"+str(args.myID), "w") as outFile:
