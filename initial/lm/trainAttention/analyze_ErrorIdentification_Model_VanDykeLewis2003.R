...
data = read.csv("/u/scr/mhahn/reinforce-logs-both-short/stimuli-full-logs-tsv-EIS/collectResults_Stims_ErrorIdentification.py_VanDyke_Lewis_2003_Model.tsv", sep="\t")
...
#ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_ToyVanDyke_Lewis_2003.pdf", height=15, width=40)
...
#ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Toy_SurprisalVanDyke_Lewis_2003.pdf", height=15, width=40)
#
...
data$That = (!grepl("nothat" , data$Condition)) - 0.5
data$Long = (!grepl("_1" , data$Condition)) - 0.5
data$Interference = ifelse(grepl("_3" , data$Condition), 0.5, ifelse(grepl("_2", data$Condition), -0.5, 0))
...
data2 = data%>%filter(Script == "errorIdentification_Erasure4.py", Region %in% c("R4_0")) %>% group_by(deletion_rate, predictability_weight, Item, Condition, That, Long, Interference) %>% summarise(EISReweighted=mean(EISReweighted), SurprisalReweighted=mean(SurprisalReweighted))

sink("output/analyze_ErrorIdentification_Model.RVanDyke_Lewis_2003.tsv")
cat("delta", "lambda", "that", "long", "interference", "AmbiguityInterference")
...
for(i in 1:nrow(config)) {
  ...
  model = (lmer(EISReweighted ~ That + Long + Interference + That*Interference +  (1|Item), data=u))
...
ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Tabor_EISVanDyke_Lewis_2003.pdf", height=10, width=15)
...
ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Tabor_SurprisalVanDyke_Lewis_2003.pdf", height=10, width=15)
