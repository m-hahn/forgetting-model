...
data = read.csv("/u/scr/mhahn/reinforce-logs-both-short/stimuli-full-logs-tsv-EIS/collectResults_Stims_ErrorIdentification.py_BartekEtal_Model.tsv", sep="\t")
...
#ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_ToyBartekEtal.pdf", height=15, width=40)
...
#ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Toy_SurprisalBartekEtal.pdf", height=15, width=40)
#
...
sink("output/analyze_ErrorIdentification_Model.RBartekEtal.tsv")
...
data2 = data%>%filter(Script == "errorIdentification_Erasure4.py", Region %in% c("Critical_0")) %>% group_by(deletion_rate, predictability_weight, Item, Condition) %>% summarise(EISReweighted=mean(EISReweighted), SurprisalReweighted=mean(SurprisalReweighted))
...
ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Tabor_EISBartekEtal.pdf", height=10, width=15)
...
ggsave(plot, file="figures/analyze_ErrorIdentification_Model_R_Tabor_SurprisalBartekEtal.pdf", height=10, width=15)
