...
configurations = configurations[sample(1:nrow(configurations)),]
   ...
   write.table(summary(model)$fixed, file=paste("slopes/", "FIXED_analyze_inter_SCRC_Avg.R_VN5.tsv", sep=""), sep="\t")          
   write.csv(slopes,file=paste("slopes/", "analyze_inter_SCRC_Avg.R_VN5.tsv", sep=""))
   model=NULL
   samples=NULL
