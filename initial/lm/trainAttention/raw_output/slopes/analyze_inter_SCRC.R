...
for(i in (1:nrow(configurations))) {
   ...
   data2 = data %>% filter(deletion_rate==delta, predictability_weight==lambda) %>% group_by(Noun, compatible.C, EmbBias.C, Item, RC.C) %>% filter(SC.C>0) %>% summarise(SurprisalReweighted=median(SurprisalReweighted)) %>% filter(RC.C>0)
   model = brm(SurprisalReweighted ~ compatible.C + EmbBias.C + EmbBias.C*compatible.C + (1+compatible.C+EmbBias.C+ compatible.C + EmbBias.C + EmbBias.C*compatible.C|Item) + (1|Noun), data=data2, cores=4, iter=5000)
   ...
   write.table(summary(model)$fixed, file=paste("slopes/", "FIXED_analyze_inter_SCRC.R_VN5_", delta, "_", lambda, ".tsv", sep=""), sep="\t")          
   write.csv(slopes,file=paste("slopes/", "analyze_inter_SCRC.R_VN5_", delta, "_", lambda, ".tsv", sep=""))
   model=NULL
   samples=NULL
