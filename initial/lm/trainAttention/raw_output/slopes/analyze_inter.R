...
for(i in (1:nrow(configurations))) {
   ...
   write.table(summary(model)$fixed, file=paste("slopes/", "FIXED_analyze_inter.R_VN5_", delta, "_", lambda, ".tsv", sep=""), sep="\t")          
   write.csv(slopes,file=paste("slopes/", "analyze_inter.R_VN5_", delta, "_", lambda, ".tsv", sep=""))
   model=NULL
   samples=NULL
