...
library(brms)
if(TRUE){
configurations = unique(data %>% select(deletion_rate, predictability_weight)) %>% filter(deletion_rate*10 == round(deletion_rate*10))
...
for(i in (1:nrow(configurations))) {
   ...
   data2 = data %>% filter(deletion_rate>=delta, deletion_rate<=delta+0.1, predictability_weight==lambda) %>% group_by(Noun, compatible.C, EmbBias.C, Item, RC.C) %>% filter(SC.C>0) %>% summarise(SurprisalReweighted=median(SurprisalReweighted))
   ...
   write.table(summary(model)$fixed, file=paste("slopes/", "FIXED_analyze_inter_Tens.R_VN5_", delta, "_", lambda, ".tsv", sep=""), sep="\t")          
   write.csv(slopes,file=paste("slopes/", "analyze_inter_Tens.R_VN5_", delta, "_", lambda, ".tsv", sep=""))
   model=NULL
   samples=NULL
