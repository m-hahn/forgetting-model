...
data = read.csv("averages_Coher.tsv", quote='"', sep="\t")
...
dataM2 = dataM2[order(dataM2$Surprisal),]

dataM$Noun = factor(dataM$Noun, levels=dataM2$Noun)

plot = ggplot(dataM, aes(x=Noun, y=Surprisal)) + geom_text(aes(label=Noun)) + facet_wrap(~deletion_rate)
write.csv(dataM2, file="output/analyze_Coher.R")
