...
data = read.csv("averages_NormJudg_Shift.tsv", quote='"', sep="\t")
...
dataV1 = dataNormJudg %>% filter(Region == "V1_0")  %>% group_by(Noun, RatioSC, predictability_weight, deletion_rate) %>% summarise(Surprisal=mean(Surprisal), ThatFraction=mean(ThatFraction))
...
ggsave("figures/surprisals_NormJudg_Shift_OnlyGramm_V1_grid.pdf", height=10, width=6)

write.table(dataV1, file="output/analyze_Judg_VizV1Surp.R.tsv", sep="\t")


ggplot(dataV1, aes(x=RatioSC, y=ThatFraction)) + geom_smooth(method="lm") + geom_point()  + geom_label(data=tValues, aes(x=-2, y=0.5, label=tValue), inherit.aes=FALSE, parse=FALSE)+ facet_grid(deletion_rate~predictability_weight, scales="free") + theme_bw() + xlab("log P(SC|the NOUN)") + ylab("V1 ThatFraction")
