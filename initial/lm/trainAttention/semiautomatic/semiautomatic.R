...
plot = ggplot(data %>% group_by(Ratio, DeletionRate, PredictabilityWeight) %>% summarise(CompleteRatio = mean(CompleteRatio, na.rm=TRUE)), aes(x=Ratio, y=CompleteRatio)) + geom_point() + geom_smooth(method="lm") + geom_label(data=tValues, aes(label=paste("t=",round(t,2)), x=(-3.75), y=c(0.8))) + facet_grid(rows=DeletionRate ~ PredictabilityWeight) + theme_bw() +  theme ( panel.grid.major = element_blank (), panel.grid.minor = element_blank ())
ggsave(plot, file="ts-table.pdf", height=8, width=8)
