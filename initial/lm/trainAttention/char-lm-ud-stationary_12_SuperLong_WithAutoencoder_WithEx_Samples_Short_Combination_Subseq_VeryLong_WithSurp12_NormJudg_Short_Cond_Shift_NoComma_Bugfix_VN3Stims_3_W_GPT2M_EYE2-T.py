...
#    print(plain_lm) 
    ...
    with torch.no_grad():
     with open("/u/scr/mhahn/reinforce-logs-both-short/calibration-full-logs-tsv/"+__file__+"_"+str(args.myID)+"_"+SANITY, "w") as outFile:
      print("\t".join(["Sentence", "Region", "TokenLower", "Itemno", "WNUM", "SentenceID", "ID", "WORD", "tokenInWord", "Surprisal", "SurprisalReweighted", "Repetition"]), file=outFile)
      TRIALS_COUNT = 0
...
#              print(resultNumeric.size(), numeric_noised.size())
              ...
              nextWord = torch.LongTensor([stoi_total.get(remainingInput[i][0], stoi_total["OOV"]) for _ in range(numberOfSamples*24)]).unsqueeze(0).cuda()
