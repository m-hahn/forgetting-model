...
#       if(length(unique(data2$ID)) == 1) {
 #         model2 = lmer(SurprisalReweighted ~ HasRC.C * compatible.C + HasRC.C * True_Minus_False.C + compatible.C + (1|Item) + (1|Noun), data=data2 %>% filter(HasSC.C>0))
  #     } else {
          model2 = lmer(SurprisalReweighted ~ True_Minus_False.C + HasRC.C + HasRC.C * True_Minus_False.C + True_Minus_False.C + compatible.C*HasRC.C + compatible.C*True_Minus_False.C + (1+True_Minus_False.C|Item) + (1|Noun), data=data2 %>% filter(HasSC.C>0) %>% group_by(compatible.C,HasRC.C, True_Minus_False.C, Item, Noun) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted)))
...
#crash()
      ...
      sink("analyze_M_VN3_248_lmer_effects.R.txt", append=TRUE)
       cat(paste(pred, del, coef(summary(model2))[2,1], coef(summary(model2))[3,1], coef(summary(model2))[4,1], "\n", sep="\t"))
       sink()
      sink("analyze_M_VN3_248_lmer_effects.R_tvalues.txt", append=TRUE)
