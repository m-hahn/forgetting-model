...
parser.add_argument("--load-from", dest="load_from", type=str, default=random.choice(["136525999", "458957841"]))
...
if args.load_from is not None:
  checkpoint = torch.load("/u/scr/mhahn/CODEBOOKS/"+args.language+"_"+__file__.replace("_GetSentenceLikelihood", "")+"_code_"+str(args.load_from)+".txt")
...
for epoch in range(1):
   ...
   sentence = "vollständig einem OOV zu behandeln aber dazu kam es nicht . der bericht für den schulleiter den er lehrer kritisierte gewann"
   numeric = torch.LongTensor([stoi_total[x] for x in sentence.split(" ")]).cuda().view(-1, 1)
   loss, numberOfCharacters = forward((numeric, None), printHere=printHere, train=False)
   print(loss)
   sentence = "vollständig einem OOV zu behandeln aber dazu kam es nicht . der bericht für den schulleiter den der lehrer kritisierte gewann"
   numeric = torch.LongTensor([stoi_total[x] for x in sentence.split(" ")]).cuda().view(-1, 1)
   loss, numberOfCharacters = forward((numeric, None), printHere=printHere, train=False)
   print(loss)
   sentence = "vollständig einem OOV zu behandeln aber dazu kam es nicht . der bericht dass der schulleiter den der lehrer kritisierte gewann"
   numeric = torch.LongTensor([stoi_total[x] for x in sentence.split(" ")]).cuda().view(-1, 1)
   loss, numberOfCharacters = forward((numeric, None), printHere=printHere, train=False)
   print(loss)
   sentence = "vollständig einem OOV zu behandeln aber dazu kam es nicht . ein bericht der als schulleiter den jüdischen lehrer kritisierte gewann"
   numeric = torch.LongTensor([stoi_total[x] for x in sentence.split(" ")]).cuda().view(-1, 1)
   loss, numberOfCharacters = forward((numeric, None), printHere=printHere, train=False)
   print(loss)
   sentence = "vollständig einem OOV zu behandeln aber dazu kam es nicht . ein bericht der als schulleiter den der lehrer kritisierte gewann"
   numeric = torch.LongTensor([stoi_total[x] for x in sentence.split(" ")]).cuda().view(-1, 1)
   loss, numberOfCharacters = forward((numeric, None), printHere=printHere, train=False)
   print(loss)
