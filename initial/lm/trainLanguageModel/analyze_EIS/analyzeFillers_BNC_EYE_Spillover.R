...
sink("output/analyzeFillers_freq_BNC_FPRT_Spillover_Averaged_New_R.tsv")
...
for(i in (1:nrow(configs))) {
   if(TRUE) { #!(ID_  %in% alreadyDone)) {
      ...
      if(nrow(data) > 0) {
         ...
         lmermodel = lmer(ReadingTime.x ~ SurprisalReweighted.x + wordInItem + LogWordFreq.x + LogWordFreq_COCA.R.x + WordLength.x + EISReweighted.x + EISReweighted.y + SurprisalReweighted.y + LogWordFreq.y + LogWordFreq_COCA.R.y + WordLength.y + ReadingTime.y + (1|itemID.x) + (1|subject), data=data %>% filter(FPRT.x > 0), REML=F)
         print(delta)
         print(lambda)
         ...
         sink("output/analyzeFillers_freq_BNC_FPRT_Spillover_Averaged_New_R.tsv", append=TRUE)
         cat(lambda, delta, nrow(data), AIC(lmermodel), coef(summary(lmermodel))[2,1], summary(lmermodel)$coef[7,3], summary(lmermodel)$coef[8,3], "\n", sep="\t")
         sink()
      }
   }
...
#ReadingTime_FPRT = log(human$FPRT)
#
...
#write.table(ReadingTime_FPRT, file="forStan_Fillers_EYE/ReadingTime_FPRT.tsv", quote=F, sep="\t")
