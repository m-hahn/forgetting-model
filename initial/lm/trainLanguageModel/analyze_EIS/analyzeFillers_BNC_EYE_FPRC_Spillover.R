...
library(lme4)
library(dplyr)
model = read.csv("/u/scr/mhahn/reinforce-logs-both-short/calibration-full-logs-tsv/collect12_NormJudg_Short_Cond_W_GPT2_ByTrial_VN3_Fillers.py.tsv", sep="\t") %>% mutate(wordInItem = Region+1) %>% mutate(item = paste("Filler", Sentence, sep="_")) %>% group_by(item, wordInItem, Region, Word, predictability_weight, deletion_rate) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted, na.rm=TRUE))
# Script, ID, 

model_EIS = read.csv("/u/scr/mhahn/reinforce-logs-both-short/calibration-full-logs-tsv-EIS/collectResults_Fillers_ErrorIdentification.py.tsv", sep="\t") %>% mutate(wordInItem = Region+1) %>% mutate(item = paste("Filler", Sentence, sep="_")) %>% group_by(item, wordInItem, Region, Word, predictability_weight, deletion_rate) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted, na.rm=TRUE), EISReweighted=mean(EISReweighted, na.rm=TRUE))
# Script, ID, 

model = merge(model, model_EIS, by=c("item", "wordInItem","Region", "Word", "predictability_weight", "deletion_rate"), all=TRUE)

print(summary(model))

# Sanity check
cor.test(model$SurprisalReweighted.x, model$SurprisalReweighted.y)
...
maze = read.csv("../../../../../resource-rational-surprisal/experiments/maze/experiment1/Submiterator-master/trials_byWord.tsv", sep="\t", quote="@") %>% filter(condition == "filler")
...
print(unique((maze_et[is.na(maze_et$rt) & !is.na(maze_et$FPRC),])$sentence))

maze_et = maze_et %>% filter(!is.na(maze_et$rt), !is.na(maze_et$FPRC))
...
sink("output/analyzeFillers_freq_BNC_FPRC_Spillover_Averaged_New_R.tsv")
...
human = data %>% select(sentence, word, wordInItem, item, subject, FPRC, TFT, RRT) %>% filter(!grepl("Practice", item), !grepl("Critical", item), wordInItem != 0) # the model has no surprisal prediction for the first word
...
for(i in (1:nrow(configs))) {
   if(TRUE) { #!(ID_  %in% alreadyDone)) {
      ...
      data = model %>% filter(deletion_rate==delta, predictability_weight==lambda) %>% group_by(wordInItem, item, itemID) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted.x), EISReweighted=mean(EISReweighted), WordLength=mean(WordLength), LogWordFreq_COCA.R=mean(LogWordFreq_COCA.R), LogWordFreq=mean(LogWordFreq))
      data = merge(data, maze_et, by=c("wordInItem", "item")) %>% filter(!is.na(LogWordFreq), !is.na(LogWordFreq_COCA.R))
      if(nrow(data) > 0) {
         data$SurprisalReweighted = resid(lm(SurprisalReweighted ~ LogWordFreq + LogWordFreq_COCA.R, data=data, na.action=na.exclude))
         data$EISReweighted = resid(lm(EISReweighted ~ SurprisalReweighted + LogWordFreq + LogWordFreq_COCA.R, data=data, na.action=na.exclude))
         data$ReadingTime = data$FPRC
         dataPrevious = data %>% mutate(wordInItem=wordInItem+1)
         data = merge(data, dataPrevious, by=c("wordInItem", "sentence", "subject"))
         lmermodel = lmer(ReadingTime.x ~ SurprisalReweighted.x + wordInItem + LogWordFreq.x + LogWordFreq_COCA.R.x + WordLength.x + EISReweighted.x + EISReweighted.y + SurprisalReweighted.y + LogWordFreq.y + LogWordFreq_COCA.R.y + WordLength.y + ReadingTime.y + (1|itemID.x) + (1|subject), data=data %>% filter(FPRC.x > 0))
         print(delta)
         print(lambda)
         print(summary(lmermodel))
         cat(lambda, delta, nrow(data), AIC(lmermodel), coef(summary(lmermodel))[2,1], summary(lmermodel)$coef[7,3], summary(lmermodel)$coef[8,3], "\n", sep="\t")
         # Print result to file
         sink("output/analyzeFillers_freq_BNC_FPRC_Spillover_Averaged_New_R.tsv", append=TRUE)
         cat(lambda, delta, nrow(data), AIC(lmermodel), coef(summary(lmermodel))[2,1], summary(lmermodel)$coef[7,3], summary(lmermodel)$coef[8,3], "\n", sep="\t")
         sink()
      }
   }
...
#ReadingTime_FPRC = log(human$FPRC)
#
...
#write.table(ReadingTime_FPRC, file="forStan_Fillers_EYE/ReadingTime_FPRC.tsv", quote=F, sep="\t")
