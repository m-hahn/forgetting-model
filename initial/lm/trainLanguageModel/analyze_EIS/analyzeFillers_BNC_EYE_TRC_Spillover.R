...
print(unique((maze_et[is.na(maze_et$rt) & !is.na(maze_et$TRC),])$sentence))

maze_et = maze_et %>% filter(!is.na(maze_et$rt), !is.na(maze_et$TRC))
...
sink("output/analyzeFillers_freq_BNC_TRC_Spillover_Averaged_New_R.tsv")
...
human = data %>% select(sentence, word, wordInItem, item, subject, TRC, TFT, RRT) %>% filter(!grepl("Practice", item), !grepl("Critical", item), wordInItem != 0) # the model has no surprisal prediction for the first word
...
for(i in (1:nrow(configs))) {
   if(TRUE) { #!(ID_  %in% alreadyDone)) {
      ...
      if(nrow(data) > 0) {
         ...
         data$ReadingTime = data$TRC
         dataPrevious = data %>% mutate(wordInItem=wordInItem+1)
         data = merge(data, dataPrevious, by=c("wordInItem", "sentence", "subject"))
         lmermodel = lmer(ReadingTime.x ~ SurprisalReweighted.x + wordInItem + LogWordFreq.x + LogWordFreq_COCA.R.x + WordLength.x + EISReweighted.x + EISReweighted.y + SurprisalReweighted.y + LogWordFreq.y + LogWordFreq_COCA.R.y + WordLength.y + ReadingTime.y + (1|itemID.x) + (1|subject), data=data %>% filter(TRC.x > 0))
         print(delta)
         print(lambda)
         ...
         sink("output/analyzeFillers_freq_BNC_TRC_Spillover_Averaged_New_R.tsv", append=TRUE)
         cat(lambda, delta, nrow(data), AIC(lmermodel), coef(summary(lmermodel))[2,1], summary(lmermodel)$coef[7,3], summary(lmermodel)$coef[8,3], "\n", sep="\t")
         sink()
      }
   }
...
#write.table(ReadingTime_TRC, file="forStan_Fillers_EYE/ReadingTime_TRC.tsv", quote=F, sep="\t")
