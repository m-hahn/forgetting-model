...
itos = [None for _ in range(5000000)]
...
output = torch.nn.Linear(args.hidden_dim, len(itos_BPE_total)).cuda()
word_embeddings = torch.nn.Embedding(num_embeddings=len(itos_BPE_total), embedding_dim=2*args.word_embedding_size).cuda()
...
def forward(numeric, train=True, printHere=False):
      ...
      loss = train_loss(log_probs.view(-1, len(itos_BPE_total)), target_tensor.view(-1))

      # Occasionally print
      if printHere:
         lossTensor = print_loss(log_probs.view(-1, len(itos_BPE_total)), target_tensor.view(-1)).view(-1, args.batchSize)
         ...
         for i in range((args.sequence_length)):
            print((losses[i][0], itos_BPE_total[numericCPU[i+1][0]], itos_BPE_total[numeric_noisedCPU[i+1][0]]))
