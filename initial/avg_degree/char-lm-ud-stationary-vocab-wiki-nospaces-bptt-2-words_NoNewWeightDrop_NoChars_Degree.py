# Based on char-lm-ud-stationary-vocab-wiki-nospaces-bptt-2-words_NoNewWeightDrop_NoChars.py
...
parser.add_argument("--load-from", dest="load_from", type=str, default=str(random.choice([921643752, 27553360])))
...
position_embedding_dim = 200

positional = torch.nn.Embedding(num_embeddings=50, embedding_dim=position_embedding_dim)

transformer1 = torch.nn.TransformerEncoder(d_model=2*args.word_embedding_size+position_embedding_dim, nhead=1, dim_feedforward=2048, dropout=0.1, activation='relu')
transformer2 = torch.nn.TransformerEncoder(d_model=2*args.word_embedding_size+position_embedding_dim, nhead=2, dim_feedforward=2048, dropout=0.1, activation='relu')
transformer3 = torch.nn.TransformerEncoder(d_model=2*args.word_embedding_size+position_embedding_dim, nhead=3, dim_feedforward=2048, dropout=0.1, activation='relu')
