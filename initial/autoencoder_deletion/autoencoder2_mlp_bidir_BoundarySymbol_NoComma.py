...
import glob
if args.load_from is not None:
  checkpoint = torch.load(glob.glob("/u/scr/mhahn/CODEBOOKS/"+args.language+"_*_code_"+str(args.load_from)+".txt")[0])
...
def forward(numeric, train=True, printHere=False):
      ...
      numeric_noised = [[x for x in y if random.random() > args.deletion_rate  or x == 1] for y in numeric.cpu().t()] # never delete the EOS symbol
